...
int mestre_fmpz(acb_poly_t crv, fmpz* IC, slong prec)
{
  ...
  /* R2 is zero */
    ...
    {
      acb_poly_set_coeff_acb(crv, 5, 1);
      acb_poly_set_coeff_acb(crv, 3, a);
      acb_poly_set_coeff_acb(crv, 1, 1);
    }
  else /* Case I */
    {
      res = cardona(crv, IC_acb, prec);
    }
...
static int bolza_conditions_19(acb_t a, fmpq* ABCD, slong prec)
{
  ...
  fmpq_pow_ui(t1, &ABCD[1], 3);
  fmpq_div_si(t1, t1, -6);
  fmpq_pow_ui(temp, &ABCD[2], 2);
  fmpq_add(t1, t1, temp);
